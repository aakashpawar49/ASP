...
const DeviceCreateModal = ({ onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!deviceName || !deviceType || !serialNumber || !status || !labId) {
      return toast.error("Please fill out all required fields.");
    }
    
    setIsSubmitting(true);
    try {
      // Call the POST /api/devices endpoint
      ...
      toast.error(error.response?.data || "Failed to create device.");
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-lg p-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-semibold text-white">Add New System</h3>
