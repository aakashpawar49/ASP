  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        // Filter for users who can be in charge (Admins or Teachers)
...
const LabCreateModal = ({ onClose, onSuccess }) => {
  const [labName, setLabName] = useState('');
  const [location, setLocation] = useState('');
  const [labInchargeId, setLabInchargeId] = useState(null);
  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!labName || !location) {
      return toast.error("Please fill out Lab Name and Location.");
    }
    ...
    try {
      ...
      toast.error(error.response?.data || "Failed to create lab.");
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-lg p-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-semibold text-white">Create New Lab</h3>
